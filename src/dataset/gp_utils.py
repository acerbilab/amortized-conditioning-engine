...
class SampleGP:
    ...
    def sample_prior(self, xpred, length_scale, sigma_f, mean_f=0, n_samples=1, correlated=True):
        """
        ...
        if correlated:
            ...
            return mean_f + torch.sqrt(var_jittered).reshape(-1, 1) * torch.randn(len(xpred), n_samples)

    def sample_posterior(self, xpred, xobs, yobs, length_scale, sigma_f, mean_f=0, n_samples=1, correlated=True):
        """
        ...
        if correlated:
            ...
            mask = var < self.jitter
            var_jittered = torch.where(mask, torch.full_like(var, self.jitter), var)
            f_post = mu.reshape(-1, 1) + torch.sqrt(var_jittered).reshape(-1, 1) * torch.randn(len(xpred), n_samples)

        return f_post

    def sample(self, xpred, xobs=None, yobs=None, length_scale=None, sigma_f=None, mean_f=0, n_samples=1, correlated=True):
        """
        ...
        if xobs is None or yobs is None or len(xobs) == 0 or len(yobs) == 0:
            return self.sample_prior(xpred, length_scale, sigma_f, mean_f, n_samples, correlated)
        else:
            return self.sample_posterior(xpred, xobs, yobs, length_scale, sigma_f, mean_f, n_samples, correlated)
...
def matern52_kernel(x1, x2, length_scale, sigma_f):
    ...
    dist = torch.norm((x1.unsqueeze(-2) - x2.unsqueeze(-3)) / length_scale, dim=-1)
    sqrt5 = math.sqrt(5.0)
    cov = sigma_f.pow(2) * (1.0 + sqrt5 * dist + 5.0 * dist.pow(2) / 3.0) * torch.exp(-sqrt5 * dist)
    return cov
