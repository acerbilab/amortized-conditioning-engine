...
def bern_unif_sampler(
    ...
    xyd, xyl = problem.get_data(
        n_total_points - num_latent, int(num_ctx), x_range, device
    )

    # image_size = 32
    # top_rows = 22
    # points_per_row = image_size
    # split_point = top_rows * points_per_row

    # index_list = [3, 4, 7, 8, 10, 14, 16, 34]
    # #[4, 5, 8, 9, 11, 17, 35]
    # #num_latent = len(index_list)

    # xyl_features = xyl[:, index_list, :]
    ...
    if latent_on_ctx and (num_latent > 1):
    ...
    elif latent_on_ctx and (num_latent == 40):
        ...
        xyc = torch.cat((xyc, xyl), dim=1)  # Include latent point in context
            # Select bottom half of the image for context
            # Select bottom 14 rows of the image for context
        # xyc = xyd[:, split_point:, :]  # [batch_size, 448, feature_dim+1]
        # # Select top 18 rows of the image for target
        # xyt = xyd[:, :split_point, :]  # [batch_size, 576, feature_dim+1]

        # xyc = torch.cat((xyc, xyl_features), dim=1)  # Include latent point in context
    ...
    batch.xc = xyc[:, :, :-3]  # Context features: [batch_size, num_ctx, feature_dim]
    batch.yc = xyc[:, :, -3:]  # Context labels: [batch_size, num_ctx, 1]

    batch.xt = xyt[:, :, :-3]  # Target features: [batch_size, num_targets, feature_dim]
    batch.yt = xyt[:, :, -3:]  # Target labels: [batch_size, num_targets, 1]
