...
class PriorSampler:
    ...
    def sample_narrow_uniform_bin_weights(self, degree):
        total_bins = int(
            degree * (self.num_bins - 1) + 1
        )  # Interpolates between 1 and num_bins
        ...
        if mode == "mixture":
            if torch.rand(1) < 0.8:
                (
                    ...
                    means,
                    stds,
                ) = self.sample_mixture_gaussian_components()
                bin_probs = torch.zeros(self.num_bins)
...
class OUP(object):
    ...
    def simulate_oup_prior_injection(
        self,
        theta_1,
        theta_2,
        mean_theta_1,
    ...
    def simulate_oup_prior_injection_bin(
        self,
        theta_1,
        theta_2,
    ...
    def get_data(self, batch_size=16):
        ...
        for i in range(batch_size):
            ...
            xc, yc, xt, yt, xyd, xyl = self.simulate_oup(
                theta_1=theta_1, theta_2=theta_2, num_points=25
            )
            batch_xc.append(xc)
            batch_yc.append(yc)
            batch_xt.append(xt)
    ...
    def get_data_prior_injection(self, batch_size=16):
        ...
        for i in range(batch_size):
            ...
            xc, yc, xt, yt, xyd, xyl = self.simulate_oup(
                theta_1, theta_2, num_points=25
            )

            batch_xc.append(xc)
            batch_yc.append(yc)
            batch_xt.append(xt)
            batch_yt_mean.append(
                torch.tensor([mean_theta_1, mean_theta_2]).reshape(-1, 1)
            )  # [2, 1]
            batch_yt_std.append(
                torch.tensor([std_theta_1, std_theta_2]).reshape(-1, 1)
            )  # [2, 1]
    ...
    def get_data_gaussian(self, batch_size=16):
        ...
        for i in range(batch_size):
            ...
            xc, yc, xt, yt, xyd, xyl_without_prior, xyl_with_prior = (
                self.simulate_oup_prior_injection(
                    theta_1,
                    theta_2,
                    mean_theta_1,
                    std_theta_1,
    ...
    def get_data_bin(
        self, batch_size=16, num_bins=100, mode="mixture", knowledge_degree=1.0
    ):
        batch = AttrDict()
        ...
        for i in range(batch_size):
            ...
            (
                ...
                xyl_with_prior_narrow,
                xyl_with_prior_wide,
            ) = self.simulate_oup_prior_injection_bin(
                theta_1,
                theta_2,
                bin_weights_1_narrow,
                bin_weights_1_wide,
            ...
            batch_yc.append(yc)
            batch_xt.append(xt)
            batch_yt_weights_narrow.append(
                torch.cat([bin_weights_1_narrow, bin_weights_2_narrow], dim=0).reshape(
                    -1, num_bins
                )
            )  # [2, 100]
            batch_yt_weights_wide.append(
                torch.cat([bin_weights_1_wide, bin_weights_2_wide], dim=0).reshape(
                    -1, num_bins
                )
            )
            batch_yt_real.append(yt)
