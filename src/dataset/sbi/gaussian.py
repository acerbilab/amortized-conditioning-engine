...
class JointPriorSampler:
    def __init__(self, num_bins, lower, upper, mean_prior, std_prior, mean_prior_another, std_prior_another, rho_prior):
    ...
    def conditional_prior(self, theta_1_value):
        ...
        """
        mu_theta_given_another = self.mean_theta + self.rho * (self.std_theta / self.std_another) * (
                theta_1_value - self.mean_another)
        sigma_thet_given_another = torch.sqrt((1 - self.rho ** 2) * self.std_theta ** 2)
        return self.gaussian_to_binned_distribution(mu_theta_given_another, sigma_thet_given_another)
...
class Gaussian(object):
    ...
    def get_data(
        ...
        for i in range(batch_size):
            ...
            sampled_points.append(self.simulate_gaussian(theta_1,
                                                         theta_2,
                                                         bin_weights_1,
                                                         bin_weights_2,
                                                         n_ctx_points))
    ...
    def get_data_with_fixed_std(
        ...
        bin_weights_1_list = [theta_1_sampler.gaussian_to_binned_distribution(mean, theta_1_fixed_std) for mean in
                              theta_1_means]
        bin_weights_2_list = [theta_2_sampler.gaussian_to_binned_distribution(theta_2_fixed_mean, theta_2_fixed_std) for
                              _ in theta_1_means]
        ...
        for i in range(batch_size):
            ...
            sampled_points.append(self.simulate_gaussian_with_fixed_data(X,
                                                         theta_1,
                                                         theta_2,
                                                         bin_weights_1,
                                                         bin_weights_2,
                                                         n_ctx_points))  # we use fixed num_points
    ...
    def get_data_with_fixed_mean(
        ...
        bin_weights_1_list = [theta_1_sampler.gaussian_to_binned_distribution(theta_1_fixed_mean, theta_1_fixed_std) for
                              _ in theta_2_means]
        bin_weights_2_list = [theta_2_sampler.gaussian_to_binned_distribution(mean, theta_2_fixed_std) for mean in
                              theta_2_means]
    ...
    def get_data_ar(self,
        ...
        batch_xyd, batch_xyl_theta_1_marginal, batch_xyl_theta_2_marginal = self.get_data_ar_marginal(num_bins=num_bins,
                                                                                                      num_ctx=num_ctx,
        ...
        batch_xyl_cond = self.get_data_ar_cond(num_bins=num_bins,
                                               num_ctx=num_ctx,
                                               joint_mean_theta_1=joint_mean_theta_1,
                                               joint_mean_theta_2=joint_mean_theta_2,
                                               joint_std_theta_1=joint_std_theta_1,
        ...
        return batch_xyd, batch_xyl_theta_1_marginal, batch_xyl_theta_2_marginal, batch_xyl_cond

    def get_data_ar_marginal(self,
                             batch_size=16,
                             num_bins=100,
                             num_ctx=20,
                             min_num_points=10,
        ...
        theta_1_bin_weights_marginal = theta_1_sampler.gaussian_to_binned_distribution(theta_1_mean,
                                                                                       theta_1_std)  # marginal prior
        theta_2_bin_weights_marginal = theta_2_sampler.gaussian_to_binned_distribution(theta_2_mean,
                                                                                       theta_2_std)  # marginal prior
        theta_1_bin_weights_flat = theta_1_sampler.sample_uniform_bin_weights()  # flat prior
        theta_2_bin_weights_flat = theta_2_sampler.sample_uniform_bin_weights()  # flat prior
        ...
        yl_weights_theta_1_marginal = torch.stack((theta_1_bin_weights_marginal, theta_2_bin_weights_flat), dim=0)
        yl_weights_theta_2_marginal = torch.stack((theta_1_bin_weights_flat, theta_2_bin_weights_marginal), dim=0)

        xyd = torch.cat((torch.full_like(xd, 1), xd, yd), dim=-1)
        latent_marker = torch.arange(2, 4).unsqueeze(-1)
        xyl_theta_1_marginal = torch.cat((latent_marker, xl, yl, yl_weights_theta_1_marginal), dim=-1)  # [Nl, 3 + 100]
        xyl_theta_2_marginal = torch.cat((latent_marker, xl, yl, yl_weights_theta_2_marginal), dim=-1)  # [Nl, 3 + 100]

        batch_xyd = xyd.unsqueeze(0)
        batch_xyl_theta_1_marginal = xyl_theta_1_marginal.unsqueeze(0)  # [1, Nl, 3 + 100]
        batch_xyl_theta_2_marginal = xyl_theta_2_marginal.unsqueeze(0)  # [1, Nl, 3 + 100]

        return batch_xyd, batch_xyl_theta_1_marginal, batch_xyl_theta_2_marginal

    def get_data_ar_cond(self,
                         batch_size=16,
                         num_bins=100,
                         num_ctx=20,
                         min_num_points=10,
        ...
        theta_1_sampler = JointPriorSampler(num_bins, -1, 1, joint_mean_theta_1, joint_std_theta_1, joint_mean_theta_2,
                                            joint_std_theta_2, joint_rho)
        theta_2_sampler = JointPriorSampler(num_bins, 0.05, 1, joint_mean_theta_2, joint_std_theta_2,
                                            joint_mean_theta_1, joint_std_theta_1, joint_rho)
        ...
        for i in range(num_bins):
            ...
            sampled_points.append(self.simulate_gaussian_for_latents(theta_1_cond,
                                                                     theta_2_cond,
                                                                     theta_1_bin_weights,
                                                                     theta_2_bin_weights,
                                                                     num_ctx))
        ...
        batch_xyl = torch.stack(
            [point for point in sampled_points], dim=0
        )

        return batch_xyl

    def simulate_gaussian_for_latents(self,
                                      theta_1,
                                      theta_2,
                                      bin_weights_1,
                                      bin_weights_2,
    ...
    def simulate_gaussian_with_fixed_data(self,
                                          X,
                                          theta_1,
                                          theta_2,
                                          bin_weights_1,
