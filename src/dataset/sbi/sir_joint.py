...
class SIR(object):
    ...
    def simulate_sir_prior_injection(
        self, beta, gamma, mean_beta, std_beta, mean_gamma, std_gamma, num_points
    ):
        S0, I0, R0 = 999999, 1, 0  # Initial conditions
    ...
    def simulate_sir_prior_injection_bin(
        self,
        beta,
        gamma,
    ...
    def get_data(self, batch_size=16):
        ...
        for i in range(batch_size):
            ...
            xc, yc, xt, yt, xyd, xyl = self.simulate_sir(
                beta=beta, gamma=gamma, num_points=10
            )
            batch_xc.append(xc)
            batch_yc.append(yc)
            batch_xt.append(xt)
    ...
    def get_data_final(self, batch_size=16):
        ...
        for i in range(batch_size):
            ...
            xc, yc, xt, yt, xyd, xyl_without_prior, xyl_with_prior = (
                self.simulate_sir_prior_injection(
                    beta=beta,
                    gamma=gamma,
                    mean_beta=mean_beta,
                    std_beta=std_beta,
        ...
        for i in range(batch_size):
            ...
            beta, bin_weights_beta_narrow, bin_weights_beta_wide = (
                theta_1_sampler.sample_theta_first_then_bin(
                    self.mean_prior_beta, beta_std_narrow, beta_std_wide
                )
            )
            gamma, bin_weights_gamma_narrow, bin_weights_gamma_wide = (
                theta_2_sampler.sample_theta_first_then_bin(
                    self.mean_prior_gamma, gamma_std_narrow, gamma_std_wide
                )
            )
            ...
            (
                ...
                xyl_with_prior_narrow,
                xyl_with_prior_wide,
            ) = self.simulate_sir_prior_injection_bin(
                beta,
                gamma,
                bin_weights_beta_narrow,
                bin_weights_beta_wide,
