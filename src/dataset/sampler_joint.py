...
class Sampler(object):
    ...
    def sample(self):
        ...
        batch_twoway = ctxtar_sampler_dict["predict_latents_fixed"](
            batch_oneway.xyd,
            batch_oneway.xyl,
            self.min_num_points,
            self.max_num_points,
    ...
    def get_obs(self):
        ...
        obs_set_twoway = ctxtar_sampler_dict["predict_latents_fixed"](
            obs_set_oneway.xyd,
            obs_set_oneway.xyl,
            self.min_num_points,
            self.max_num_points,
    ...
    def sample_ppd(self, sampling_way=1, know_theta=False):
        ...
        batch_twoway = uniform_sampler_one_side(
            batch_everything.xyd,
        ...
        batch_twoway_pi_narrow = uniform_sampler_one_side_pi_bin(
            batch_everything.xyd,
        ...
        batch_twoway_pi_wide = uniform_sampler_one_side_pi_bin(
            batch_everything.xyd,
        ...
        return (
            batch_twoway,
            batch_twoway_pi_narrow,
            batch_twoway_pi_wide,
        )  # one way is not needed
    ...
    def sample_all_bin(self, mode="mixture", knowledge_degree=1.0):
        ...
        batch_twoway_pi_narrow = ctxtar_sampler_dict[
            "predict_latents_fixed_twoway_bin"
        ](
            batch_everything.xyd,
            batch_everything.xyl_with_prior_narrow,
            self.min_num_points,
        ...
        batch_twoway_pi_wide = ctxtar_sampler_dict["predict_latents_fixed_twoway_bin"](
            batch_everything.xyd,
            batch_everything.xyl_with_prior_wide,
            self.min_num_points,
            self.max_num_points,
...
def uniform_sampler_one_side(
    ...
    if sampling_way == 1:
        num_ctx = torch.randint(
            low=min_num_points, high=max_num_points, size=[1]
        ).item()
        xyc = xyd[:, :num_ctx, :]
        xyt = xyd[:, num_ctx:, :]
    elif sampling_way == 2:
    ...
    if know_theta:
        ctx_mask = torch.randint(
            1, 2, (num_latent,), dtype=torch.bool
        )  # always true, latents always in context
    else:
...
def uniform_sampler_predict_latents_fixed_twoway(
    ...
    # now for latents
    ctx_mask = torch.randint(
        0, 1, (num_latent,), dtype=torch.bool
    )  # change from 2 to 1 here to make sure we predict all latents
    latent_known = batch_xyl[:, ctx_mask, :]
    latent_unknown = batch_xyl[:, ~ctx_mask, :]
    ...
    context_known[:, :, 3] = torch.tensor([0.0])  # std set to 0
    ...
    batch.yc_data = data_in_ctx[:, :, -1:]  # [B, Nc, 1] (y for data) NEED
    batch.yc_latent_mean = latent_in_ctx[:, :, 2].unsqueeze(
        -1
    )  # [B, Nl, 1] latent mean NEED
    batch.yc_latent_std = latent_in_ctx[:, :, 3].unsqueeze(
        -1
    )  # [B, Nl, 1] latent std NEED
    ...
    batch.xc = torch.cat(
        [batch.xc_data, batch.xc_latent], dim=1
    )  # [B, Nc+Nl, 2] (marker, 0 for latent and x for data) NEED
