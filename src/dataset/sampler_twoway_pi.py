...
def bern_unif_sampler(problem,
    ...
    if num_ctx == "random":
        num_ctx = torch.randint(low=min_ctx_points, high=max_ctx_points + 1, size=[1]).item()
    ...
    if latent_on_ctx and (num_latent > 1):
        ...
        num_latent_ctx = torch.randint(1, num_latent + 1, (xyd.shape[0],))  # [batch_size]
        ...
        batch.yc_latent_known = context_known[:, :, [2]]  # [B, n_known, 1] true value for known latents

        batch.xc_latent_unknown = context_unknown[:, :, :2]  # [B, n_unknown, 2]
        batch.bins_latent_unknown = context_unknown[:, :, 2:]  # [B, n_unknown, 100] bin weights for unknown latents

        batch.xc = torch.cat([batch.xc_data, batch.xc_latent_known, batch.xc_latent_unknown], dim=1)  # [B, Nc+Nl, 2]
        ...
        batch.yc_latent_known = xyl[:, :, [2]]  # [B, n_known, 1] true value for known latents
        ...
        batch.bins_latent_unknown = xyl[:, :, 3:]  # [B, n_unknown, 100] bin weights for unknown latents
...
def uniform_sampler_one_side(problem,
                             batch_size,
                             num_ctx,
                             num_latent,
                             min_ctx_points,
    ...
    # latents in target set
    latent_in_tar = torch.zeros(latent_unknown.size(0), latent_unknown.size(1), 3).to(device)
    latent_in_tar[:, :, 0:3] = latent_unknown[:, :, 0:3]  # discard bin weights
...
def sample_for_gaussian(problem=None,
                        batch_size=16,
                        num_ctx=10,
                        num_latent=2,
                        min_ctx_points=10,
    ...
    if num_ctx == "random":
        num_ctx = torch.randint(low=min_ctx_points, high=max_ctx_points, size=[1]).item()

    if mode == "default":
        batch_xyd, batch_xyl = problem.get_data(batch_size=batch_size,
                                                n_total_points=None,
                                                n_ctx_points=int(num_ctx),
                                                x_range=None,
                                                num_bins=num_bins,
        ...
        batch_xyd, batch_xyl = problem.get_data_with_fixed_std(batch_size=batch_size,
                                                               n_total_points=None,
                                                               n_ctx_points=int(num_ctx),
                                                               x_range=None,
                                                               num_bins=num_bins,
        ...
        batch_xyd, batch_xyl = problem.get_data_with_fixed_mean(batch_size=batch_size,
                                                                n_total_points=None,
                                                                n_ctx_points=int(num_ctx),
                                                                x_range=None,
                                                                num_bins=num_bins,
    ...
    num_ctx_for_data = torch.randint(low=min_ctx_points, high=max_ctx_points, size=[1]).item()
    ...
    latent_in_tar_mean = torch.zeros(latent_unknown_mean.size(0), latent_unknown_mean.size(1), 3)
    ...
    latent_in_tar_std = torch.zeros(latent_unknown_std.size(0), latent_unknown_std.size(1), 3)
    ...
    batch.xc_data = batch_know_mean.xc_data = batch_know_std.xc_data = data_in_ctx[:, :, :2]  # [B, Nc, 2]
    batch.yc_data = batch_know_mean.yc_data = batch_know_std.yc_data = data_in_ctx[:, :,
                                                                       -1:]  # [B, Nc, 1] (y for data) NEED
    ...
    batch_know_mean.xc = torch.cat(
        [batch.xc_data, batch_know_mean.xc_latent_known, batch_know_mean.xc_latent_unknown], dim=1)

    batch_know_std.xc_latent_known = context_known_std[:, :, :2]
    batch_know_std.xc_latent_unknown = context_unknown_std[:, :, :2]
    batch_know_std.xc = torch.cat([batch.xc_data, batch_know_std.xc_latent_known, batch_know_std.xc_latent_unknown],
                                  dim=1)

    return batch, batch_know_mean, batch_know_std


def sample_ar(problem,
              num_ctx=20,
              ...
              joint_rho=0.5,
              num_bins=100):
    batch_xyd, batch_xyl_theta_1_marginal, batch_xyl_theta_2_marginal, batch_xyl_cond = problem.get_data_ar(num_bins,
                                                                                                            num_ctx,
                                                                                                            joint_mean_theta_1,
                                                                                                            joint_mean_theta_2,
                                                                                                            joint_std_theta_1,
    ...
    ctx_mask_theta_1_marginal = torch.tensor([False, False], dtype=torch.bool)  # pred all, but only take theta 1
    ctx_mask_theta_2_marginal = torch.tensor([False, False], dtype=torch.bool)  # pred all, but only take theta 2

    ctx_mask_theta_1_cond = torch.tensor([True, False], dtype=torch.bool)  # pred theta 2, given theta 1
    ctx_mask_theta_2_cond = torch.tensor([False, True], dtype=torch.bool)  # pred theta 1, given theta 2

    latent_known_theta_1_marginal = batch_xyl_theta_1_marginal[:, ctx_mask_theta_1_marginal, :]
    latent_known_theta_2_marginal = batch_xyl_theta_2_marginal[:, ctx_mask_theta_2_marginal, :]
    latent_unknown_theta_1_marginal = batch_xyl_theta_1_marginal[:, ~ctx_mask_theta_1_marginal, :]
    latent_unknown_theta_2_marginal = batch_xyl_theta_2_marginal[:, ~ctx_mask_theta_2_marginal, :]

    latent_known_theta_1_cond = batch_xyl_cond[:, ctx_mask_theta_1_cond, :]  # condition on grid theta 1, pred theta 2
    latent_known_theta_2_cond = batch_xyl_cond[:, ctx_mask_theta_2_cond, :]  # condition on grid theta 2, pred theta 1
    latent_unknown_theta_1_cond = batch_xyl_cond[:, ~ctx_mask_theta_1_cond, :]  # not important
    latent_unknown_theta_2_cond = batch_xyl_cond[:, ~ctx_mask_theta_2_cond, :]  # not important

    context_known_theta_1_marginal = latent_known_theta_1_marginal[:, :, :3]  # marker, 0, true value, should be none
    context_known_theta_2_marginal = latent_known_theta_2_marginal[:, :, :3]  # marker, 0, true value, should be none

    context_known_theta_1_cond = latent_known_theta_1_cond[:, :, :3]  # marker, 0, true value
    context_known_theta_2_cond = latent_known_theta_2_cond[:, :, :3]  # marker, 0, true value

    # marginal theta 1 + flat theta 2
    context_unknown_theta_1_marginal = torch.zeros_like(latent_unknown_theta_1_marginal[:, :, :-1])
    context_unknown_theta_1_marginal[:, :, 0] = latent_unknown_theta_1_marginal[:, :, 0]  # latent marker
    context_unknown_theta_1_marginal[:, :, 1] = latent_unknown_theta_1_marginal[:, :, 1]  # 0
    context_unknown_theta_1_marginal[:, :, 2:] = latent_unknown_theta_1_marginal[:, :, 3:]

    # marginal theta 2 + flat theta 1
    context_unknown_theta_2_marginal = torch.zeros_like(latent_unknown_theta_2_marginal[:, :, :-1])
    context_unknown_theta_2_marginal[:, :, 0] = latent_unknown_theta_2_marginal[:, :, 0]  # latent marker
    context_unknown_theta_2_marginal[:, :, 1] = latent_unknown_theta_2_marginal[:, :, 1]  # 0
    context_unknown_theta_2_marginal[:, :, 2:] = latent_unknown_theta_2_marginal[:, :, 3:]

    # conditional prior for theta 2
    context_unknown_theta_1_cond = torch.zeros_like(latent_unknown_theta_1_cond[:, :, :-1])
    ...
    context_unknown_theta_2_cond = torch.zeros_like(latent_unknown_theta_2_cond[:, :, :-1])
    ...
    latent_in_tar_theta_1_marginal = torch.zeros(latent_unknown_theta_1_marginal.size(0),
                                                 latent_unknown_theta_1_marginal.size(1), 3)
    latent_in_tar_theta_1_marginal[:, :, 0:3] = latent_unknown_theta_1_marginal[:, :, 0:3]
    latent_in_tar_theta_2_marginal = torch.zeros(latent_unknown_theta_2_marginal.size(0),
                                                 latent_unknown_theta_2_marginal.size(1), 3)
    latent_in_tar_theta_2_marginal[:, :, 0:3] = latent_unknown_theta_2_marginal[:, :, 0:3]

    latent_in_tar_theta_1_cond = torch.zeros(latent_unknown_theta_1_cond.size(0), latent_unknown_theta_1_cond.size(1),
                                             3)
    latent_in_tar_theta_1_cond[:, :, 0:3] = latent_unknown_theta_1_cond[:, :, 0:3]
    latent_in_tar_theta_2_cond = torch.zeros(latent_unknown_theta_2_cond.size(0), latent_unknown_theta_2_cond.size(1),
                                             3)
    latent_in_tar_theta_2_cond[:, :, 0:3] = latent_unknown_theta_2_cond[:, :, 0:3]

    xyt_theta_1_marginal = torch.concat((data_in_tar, latent_in_tar_theta_1_marginal), dim=1)
    xyt_theta_2_marginal = torch.concat((data_in_tar, latent_in_tar_theta_2_marginal), dim=1)

    xyt_theta_1_cond = torch.concat((data_in_tar_for_cond, latent_in_tar_theta_1_cond), dim=1)
    xyt_theta_2_cond = torch.concat((data_in_tar_for_cond, latent_in_tar_theta_2_cond), dim=1)

    batch_theta_1_marginal.xt = xyt_theta_1_marginal[:, :, :-1]  # [B, Nt, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_1_marginal.yt = xyt_theta_1_marginal[:, :, -1:]  # [B, Nt, 1] (y for data and theta) NEED

    batch_theta_2_marginal.xt = xyt_theta_2_marginal[:, :, :-1]  # [B, Nt, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_2_marginal.yt = xyt_theta_2_marginal[:, :, -1:]  # [B, Nt, 1] (y for data and theta) NEED

    batch_theta_1_cond.xt = xyt_theta_1_cond[:, :, :-1]  # [B, Nt, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_1_cond.yt = xyt_theta_1_cond[:, :, -1:]  # [B, Nt, 1] (y for data and theta) NEED

    batch_theta_2_cond.xt = xyt_theta_2_cond[:, :, :-1]  # [B, Nt, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_2_cond.yt = xyt_theta_2_cond[:, :, -1:]  # [B, Nt, 1] (y for data and theta) NEED

    batch_theta_1_marginal.yc_latent_known = context_known_theta_1_marginal[:, :,
                                             [2]]  # [B, n_known, 1] true value NEED
    batch_theta_1_marginal.bins_latent_unknown = context_unknown_theta_1_marginal[:, :,
                                                 2:]  # [B, n_unknown, 100] latent mean NEED
    ...
    batch_theta_1_cond.yc_latent_known = context_known_theta_1_cond[:, :, [2]]  # [B, n_known, 1] true value NEED
    batch_theta_1_cond.bins_latent_unknown = context_unknown_theta_1_cond[:, :,
                                             2:]  # [B, n_unknown, 100] latent mean NEED

    batch_theta_2_cond.yc_latent_known = context_known_theta_2_cond[:, :, [2]]  # [B, n_known, 1] true value NEED
    ...
    batch_theta_1_marginal.xc_latent_known = context_known_theta_1_marginal[:, :, :2]  # [B, n_known, 2]
    batch_theta_1_marginal.xc_latent_unknown = context_unknown_theta_1_marginal[:, :, :2]  # [B, n_unknown, 2]

    batch_theta_2_marginal.xc_latent_known = context_known_theta_2_marginal[:, :, :2]  # [B, n_known, 2]
    batch_theta_2_marginal.xc_latent_unknown = context_unknown_theta_2_marginal[:, :, :2]  # [B, n_unknown, 2]

    batch_theta_1_cond.xc_latent_known = context_known_theta_1_cond[:, :, :2]  # [B, n_known, 2]
    batch_theta_1_cond.xc_latent_unknown = context_unknown_theta_1_cond[:, :, :2]

    batch_theta_2_cond.xc_latent_known = context_known_theta_2_cond[:, :, :2]  # [B, n_known, 2]
    batch_theta_2_cond.xc_latent_unknown = context_unknown_theta_2_cond[:, :, :2]

    batch_theta_1_marginal.xc = torch.cat([batch_theta_1_marginal.xc_data, batch_theta_1_marginal.xc_latent_known,
                                           batch_theta_1_marginal.xc_latent_unknown],
                                          dim=1)  # [B, Nc+Nl, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_2_marginal.xc = torch.cat([batch_theta_2_marginal.xc_data, batch_theta_2_marginal.xc_latent_known,
                                           batch_theta_2_marginal.xc_latent_unknown],
                                          dim=1)  # [B, Nc+Nl, 2] (marker, 0 for latent and x for data) NEED

    batch_theta_1_cond.xc = torch.cat(
        [batch_theta_1_cond.xc_data, batch_theta_1_cond.xc_latent_known, batch_theta_1_cond.xc_latent_unknown],
        dim=1)  # [B, Nc+Nl, 2] (marker, 0 for latent and x for data) NEED
    batch_theta_2_cond.xc = torch.cat(
        [batch_theta_2_cond.xc_data, batch_theta_2_cond.xc_latent_known, batch_theta_2_cond.xc_latent_unknown],
        dim=1)  # [B, Nc+Nl, 2] (marker, 0 for latent and x for data) NEED

    return batch_theta_1_marginal, batch_theta_1_cond, batch_theta_2_marginal, batch_theta_2_cond
